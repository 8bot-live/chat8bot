...
function chat8bot_handle_ajax_request() {
    ...
    if (intval($gpt_msg_memory) > 0 && file_exists($chat_file)) {
        ...
        file_put_contents("{$dev_dir}/chat_history_slice_$gpt_msg_memory.txt", json_encode($chat_data, JSON_PRETTY_PRINT));
    ...
    //save curl questions
    file_put_contents("{$dev_dir}/last_curl1_request.txt", json_encode($request_data, JSON_PRETTY_PRINT));
    ...
    //save curl response
    file_put_contents("{$dev_dir}/last_curl2_response.txt", json_encode(array_merge($decoded_response), JSON_PRETTY_PRINT));
